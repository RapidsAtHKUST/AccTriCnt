...
edge_lst_type ReadEdgeList(string &dir) {
    ...
    edge_lst_type edges2;
    // TODO: adopt parallel sort
#ifdef TBB
    auto tmp1 = high_resolution_clock::now();
    ...
    log_info("parallel sort time: %.3lf s", duration_cast<milliseconds>(tmp - tmp1).count() / 1000.0);
#else

    sort(edges.begin(), edges.end(), [](const pair<int32_t, int32_t> &left, const pair<int32_t, int32_t> &right) {
        if (left.first == right.first) {
            return left.second < right.second;
        }
        return left.first < right.first;
...
#pragma omp critical
    ...
    string reorder_deg_file_path = dir + "/" + "b_degree.bin";
    string reorder_adj_file_path = dir + "/" + "b_adj.bin";
