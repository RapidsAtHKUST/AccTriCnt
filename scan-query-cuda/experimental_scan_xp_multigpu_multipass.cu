...
#pragma omp parallel for num_threads(thread_num_/2) schedule(dynamic, 60000)
    ...
    log_debug("Num of GPUs: %d", num_of_gpus);
	...
	extern int num_pass_global;
	// 2nd: submit tasks
#if defined(USE_BITMAP_KERNEL)
	const uint32_t TITAN_XP_WARP_SIZE = 32;
	uint32_t block_size = num_of_warps_global * TITAN_XP_WARP_SIZE;
	dim3 t_dimension(WARP_SIZE, num_of_warps_global); /*2-D*/
#endif
	...
	int num_of_pass = num_pass_global;
...
#pragma omp parallel num_threads(num_of_gpus)
	{
		...
		uint32_t conc_blocks_per_SM = min(max_threads_per_SM / block_size, 16); /*assume regs are not limited*/
...
#pragma omp single nowait
		...
		for (auto pass_id = start_pass_id; pass_id < num_of_pass; pass_id++) {
...
#if	defined(USE_BITMAP_KERNEL)  && defined(UNIFIED_MEM) && defined(BITMAP_1D)
			...
			auto skew_ratio = 50;
			auto num_warps = num_of_warps_global;
			auto warp_size = 32;
